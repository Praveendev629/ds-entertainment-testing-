...
function hasMovieAnchors(html: string): boolean {
  ...
  while ((m = anchorRe.exec(html)) !== null) {
    ...
    if (href.startsWith("http") && !href.includes("isaidub.green") && !href.includes("moviesda31.com")) continue;
...
function extractMoviesFromPage(
  ...
  if (movies.length === 0) {
    ...
    const movieCandidates = candidates.filter(c => {
      ...
      const isInternal = url.startsWith("/") || url.includes("moviesda31.com") || url.includes("animesalt.cx");
      const isLetterOrPage = !!url.match(/\/[a-z]([\/#]|$)/) || !!url.match(/\/page\/(\d+)/);
      const isNotIndex = !isLetterOrPage;
      
      if (site === "moviesda") {
        return isInternal && isNotIndex && (isMovieUrl || hasYear || url.includes("tamil-movies") || url.includes("moviesda31.com/tamil-movies"));
      }
